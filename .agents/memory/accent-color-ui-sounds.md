...
- `applyUiAccent()` in appearance-settings.ts sets CSS var `--blinga-ui-accent` on documentElement and adds/removes `blinga-accent-on` class on body.
- CSS rule in index.css: `.blinga-accent-on [data-state=checked]` overrides switch color; `.blinga-accent-on .user-msg-bubble` overrides user bubble bg.
...
## UI Sounds
- `playTabClick()` uses a native short WAV tap first (reliable in Replit iframes), then Web Audio as fallback; it respects `uiSoundsEnabled`.
- Respects `localStorage.getItem('uiSoundsEnabled') === 'false'` (default: enabled).
- Hooked to: main tab bar (changeTab), Nomad Multi/Ultimatum buttons, Blinga Minds category pills, Games leaderboard filter, Settings sidebar tabs, Theme SlidingPillSelector, Message bar style SlidingPillSelector.
