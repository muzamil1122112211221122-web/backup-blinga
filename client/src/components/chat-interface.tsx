  ...
  return (
    ...
    >
                                      ...
                                      <TooltipTrigger asChild>
                                        <DropdownMenuTrigger asChild>
                                          <Button variant="ghost" className="w-10 h-10 text-zinc-400 bg-zinc-200/70 hover:text-white hover:bg-white/10 rounded-full transition-all flex-shrink-0 p-0">
                                            <img src={resolvedTheme === "dark" ? "/plus-gray.png" : "/plus-black.png"} style={{ opacity: 1, width: 18, height: 18 }} alt="attach" />
                                          </Button>
                                        </DropdownMenuTrigger>
                                      </TooltipTrigger>
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <Button variant="ghost" size="icon"
                                        ...
                                        <img src={resolvedTheme === "dark" ? "/mic-icon-dark.png" : "/mic-icon-light.png"} alt="Mic" className="w-4 h-4" style={{ filter: resolvedTheme === "dark" ? "none" : "brightness(0)" }} />
                                      </Button>
                                    </TooltipTrigger>
                                      ...
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <Button variant="ghost" size="icon"
                                            ...
                                            onClick={toggleListening} disabled={!speechSupported} data-testid="button-mic">
                                            <img src={resolvedTheme === "dark" ? "/mic-icon-dark.png" : "/mic-icon-light.png"} alt="Mic" className="w-5 h-5" style={{ filter: resolvedTheme === "dark" ? "none" : "brightness(0)" }} />
                                          </Button>
                                        </TooltipTrigger>
                                        <TooltipContent>{isListening ? 'Stop listening' : 'Voice input'}</TooltipContent>
                                      </Tooltip>
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <Button variant="ghost" size="icon" className="w-9 h-9 rounded-full text-zinc-400 bg-zinc-200/70 hover:text-white hover:bg-white/10"
                                            ...
                                            {isEnhancing ? <div className="animate-spin w-5 h-5 border-2 border-zinc-400 border-t-transparent rounded-full" /> : <img src={resolvedTheme === "dark" ? "/enhance-icon-dark.png" : "/enhance-icon-light.png"} alt="Enhance" className="w-5 h-5" style={{ filter: resolvedTheme === "dark" ? "none" : "brightness(0)" }} />}
                                          </Button>
                                        </TooltipTrigger>
        ...
        return (
          <div className={`macos-function-bar ${!askFnBarCentered ? 'macos-function-bar-with-messages' : ''} rounded-3xl mx-3 sm:mx-4 mb-1 max-w-[50rem] mx-auto w-full !border-none !shadow-none ${activeTab === 'philosopher' || activeTab === 'blinga-games' || activeTab === 'blinga-labs' || activeTab === 'imagine' || activeTab === 'nomad' || functionBarStyle === 'message-bar' || isVoiceModeModalOpen || isVoiceModeOpen ? 'hidden' : ''}`} style={askFnBarCentered ? {width: 'fit-content', position: 'fixed', top: isPill ? 'calc(50% - 48px)' : 'calc(50% - 68px)', left: (isSidebarOpen && sidebarOpenMode === 'mini') ? 'calc(50vw + 38px)' : '50vw', transform: isPill ? 'translateX(calc(-50% - 14px))' : 'translateX(-50%)', zIndex: 20, marginBottom: '10px'} : {width: 'fit-content', marginLeft: 'auto', marginRight: 'auto', marginTop: isPill ? '43px' : '11px', marginBottom: '14px', transform: isPill ? 'translateX(-14px)' : undefined, position: 'relative', zIndex: 30}}>
            <div className={`flex flex-wrap justify-center p-3 bg-transparent !border-none ${isPill ? 'gap-2' : 'gap-4'}`}>
              ...
              {renderFunctionBtn(
                <img src={resolvedTheme === 'dark' ? '/fn-settings-gray.png' : '/fn-settings-black.png'} alt="Voice Mode" className="btn-icon" style={{ opacity: 1, width:'26px',height:'26px' }} />,
                'Voice Mode',
                openVoiceMode,
                undefined,
              ...
              {renderFunctionBtn(
                <img src={resolvedTheme === 'dark' ? '/fn-longans-gray.png' : '/fn-longans-black.png'} alt="Settings" className="btn-icon" style={{ opacity: 1, width:'26px',height:'26px' }} />,
                'Settings',
      ...
      <div data-message-bar className={`max-w-[48rem] w-full px-4 ${((activeTab === 'ask' && messages.length > 0) || (activeTab === 'nomad' && Object.values(nomadMessages).some(msgs => msgs.length > 0)) || (activeTab === 'philosopher' && philosopherMessages.length > 0)) ? 'message-composer-with-messages' : ''} ${(activeTab === 'ask' && messages.length === 0) || (activeTab === 'nomad' && nomadMode === 'multi' && Object.values(nomadMessages).every(msgs => msgs.length === 0)) ? 'absolute lg:fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20' : (activeTab === 'philosopher' && selectedPersonality && philosopherMessages.length === 0) ? 'absolute lg:fixed left-1/2 -translate-x-1/2 -translate-y-1/2 z-20' : 'flex-shrink-0 mx-auto mb-4 sm:mb-8'} ${activeTab === 'blinga-games' || activeTab === 'blinga-labs' || activeTab === 'imagine' || (activeTab === 'philosopher' && !selectedPersonality) || isVoiceModeModalOpen || isVoiceModeOpen ? 'hidden' : ''}`} style={
        ...
        {replyQuote && (
          <div className="reply-chip-in flex items-center gap-0 mb-2 w-fit max-w-xs overflow-hidden rounded-xl
            ...
            <div className="flex items-center gap-2 px-2.5 py-1.5 min-w-0">
              ...
              <span className="truncate text-[11px] text-zinc-600 dark:text-zinc-300 italic leading-snug">
                {replyQuote.slice(0, 60)}{replyQuote.length > 60 ? '…' : ''}
              </span>
              <button
                onClick={() => { setReplyQuote(null); setReplyBtnPos(null); }}
                ...
                <DropdownMenu open={attachOpen} onOpenChange={setAttachOpen}>
                  <TooltipTrigger asChild>
                    <DropdownMenuTrigger asChild>
                      ...
                      >
                        <img src={resolvedTheme === "dark" ? "/plus-gray.png" : "/plus-black.png"} style={{ opacity: 1, width: 18, height: 18, transform: attachOpen ? "rotate(45deg)" : "rotate(0deg)", transition: "transform 0.25s cubic-bezier(0.4, 0, 0.2, 1)" }} alt="attach" />
                      </Button>
                    </DropdownMenuTrigger>
                  </TooltipTrigger>
                   <DropdownMenuContent className="bg-white dark:bg-[#383838] !bg-white dark:!bg-[#383838] border-none text-black dark:text-white rounded-xl shadow-2xl p-1 min-w-[190px]">
                    ...
                    <TooltipTrigger asChild>
                      ...
                      >
                        {isEnhancing ? (
                          ...
                          <img src={resolvedTheme === "dark" ? "/enhance-icon-dark.png" : "/enhance-icon-light.png"} alt="Enhance" className="w-5 h-5" style={{ filter: resolvedTheme === "dark" ? "none" : "brightness(0)" }} />
                        )}
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Enhance prompt</TooltipContent>
                      ...
                      >
                        {isEnhancing ? (
                          ...
                          <img
                            ...
                            alt="Enhance"
                            className="w-5 h-5" style={{ filter: resolvedTheme === "dark" ? "none" : "brightness(0)" }} />
                        )}
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Enhance prompt</TooltipContent>
