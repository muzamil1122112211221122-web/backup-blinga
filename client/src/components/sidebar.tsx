...
import { Plus, Trash2, X, Check, Search, Mic, Image as ImageIcon, FolderOpen, History, ChevronLeft, Settings as LucideSettings, Edit3 as PenTool } from "lucide-react";
...
export function Sidebar({
  ...
  const projectItems = projects.filter(p => p.isProject);
  const chatItems = projects.filter(p => !p.isProject);

  const projectGroups = groupItemsByDate(projectItems);
                ...
                {Object.entries(projectGroups).map(([groupName, groupProjects]) => (
                  groupProjects.length > 0 && (
                    <div key={groupName} className="space-y-1">
                      ...
                      <div className="space-y-1">
                        {(showAllGroups.has(groupName + "_project") ? groupProjects : groupProjects.slice(0, 5)).map((project) => (
                          <div
                            key={project.id}
                            className={`group relative px-3 py-2 rounded-lg transition-all duration-200 cursor-pointer ${
                              currentProjectId === project.id
                                ...
                                : 'hover:bg-zinc-50 dark:hover:bg-zinc-900/30 text-zinc-600 dark:text-zinc-400'
                            }`}
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {hoveredProject === project.id && editingProject !== project.id && (
                                <div className="flex items-center space-x-1 ml-2">
                                  <button onClick={(e) => { e.stopPropagation(); setEditTitle(project.title); setEditingProject(project.id); }} className="p-1 hover:text-blue-500"><PenTool className="h-3 w-3" /></button>
                                  <button onClick={(e) => { e.stopPropagation(); onDeleteProject(project.id); }} className="p-1 hover:text-red-500"><Trash2 className="h-3 w-3" /></button>
                                </div>
                              )}
                            </div>
                          </div>
